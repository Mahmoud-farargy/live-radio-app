...
const AdvancedSearch = lazy(() =>
  retry(() => import("../AdvancedSearch/AdvancedSearch")),
)
...
const DIV = styled.div`
  ${({ $playerState }) => {
    const isPlayerVisible =
      $playerState.isPlayerFullMode && $playerState.isPlayerOpen

    return `
      height: calc(
        100vh - var(--height-header) -
        ${isPlayerVisible ? "var(--player-height-size)" : "0px"}
      );
      overflow-y: auto;
...
const Sidebar = ({
  ...
  const menuList = Object.freeze([
    {
      id: "oif98eubf",
      title: t("menuList.main.title"),
      icon: <CgMenuGridO />,
      description: "",
    },
    {
      id: "w0fun0ufg",
      title: t("menuList.advanced_search.title"),
      icon: <RiSearchEyeLine />,
      description: t("menuList.advanced_search.description"),
    },
    {
      id: "foiw9fy9w",
      title: t("menuList.settings.title"),
      icon: <FiSettings />,
      description: `${t("menuList.settings.description.pt1")}${appConfig.title}${t("menuList.settings.description.pt2")}`,
    },
    {
      id: "wouobwfgg",
      title: t("menuList.hotkeys.title"),
      icon: <BsKeyboard />,
      description: t("menuList.hotkeys.description"),
    },
  ])
  ...
  const onSubmission = (d) => {
    d.preventDefault()
    navigate(
      `${consts.SEARCH}?${serialize({ name: searchVal, order: "clickcount" })}`,
    )
    closeSidebarOnMobile()
  }
  ...
  return (
    <Fragment>
      <aside id="sidebar">
        <DIV
          $playerState={{ isPlayerFullMode, isPlayerOpen }}
          className="sidebar-container flex-column"
        >
          <div className="sidebar--inner">
            <div className="search">
              <form
                className="search-bar flex-row"
                onSubmit={(d) => onSubmission(d)}
              >
                <input
                  className="search--input primary__input"
                  type="text"
                  name="search"
                  value={searchVal}
                  onChange={(e) => setSearchVal(e.target.value)}
                  placeholder={`${t("inputPlaceholders.searchBar")}..`}
                  spellCheck="yes"
                />
                ...
                <ul className="menu--section--inner flex-row">
                  {menuList &&
                    ...
                    menuList.map((menuItem) => {
                      return (
                        <li
                          onClick={() => handleMenuItemClicking(menuItem)}
                          key={menuItem.id}
                          className={`${activeListItem.id === menuItem.id ? "active--menu--item" : ""} menu__section__item tooltip`}
            ...
            <div className="current--active--menu">
              <h5 className="current--active--menu--title">
                {activeListItem?.title}
              </h5>
              <small className="current--active--menu--title">
              ...
              <div className="current--active--menu--inner">
                ...
                {activeListItem.id === consts.menuIds.main ? (
                  ...
                  <ul onClick={(e) => detectClicking(e)}>
                    ...
                    <li
                      title={t("menu.podcasts")}
                      data-title={t("menu.podcasts")}
                      className="tooltip"
                    >
                      <NavLink
                        className={({ isActive }) =>
                    ...
                    <li
                      title={t("menu.recently_played")}
                      data-title={t("menu.recently_played")}
                      className="tooltip"
                    >
                      <NavLink
                        className={({ isActive }) =>
                      ...
                      >
                        ...
                        <span className="nav-item-name">
                          {t("menu.recently_played")}
                        </span>
                      </NavLink>
                    </li>
                    <li
                      title={t("menu.about")}
                      data-title={t("menu.about")}
                      className="tooltip"
                    >
                      <NavLink
                        className={({ isActive }) =>
          ...
          <span className="copyright__sidebar">
            &copy; 2021 Mahmoud Farargy
          </span>
        </DIV>
      </aside>
    </Fragment>
